...
**09:33 UTC 启动与恢复更新**：后台和新建公网隧道现在由本地 Windows CIM 隐藏启动，不依赖启动它的工具进程组，沿用同一用户和原安全停止脚本。没有安装系统服务或计划任务。后台日志仍在 `.runtime/solo.stdout.log` / `solo.stderr.log`，worker 自身异常在 `solo.worker.stderr.log`；新隧道自写 `tunnel.log` 并保留旧日志归档。页面失联时每 10 秒重试，恢复后按来源清除旧提示；认证失败需重新从桌面打开。临时隧道进程存活不等于公网正常：若注册报 Tunnel not found，应在无活动通话时停止重建，并核验、重新绑定 Twilio 回调。本轮已完成该恢复，当前地址只留本机。
