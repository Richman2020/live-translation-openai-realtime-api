...
**2026-09-23 当前状态：7 项供应商设置已全部保存，`check:solo` 的 10 项必需设置全部 `ready`，正式 `verify:providers` 4/4 通过，号码语音回调已改绑并回读核验，浏览器注册通过。** 首轮拨号超时已进一步定位到麦克风授权/准备受阻；新增预检后再建会话，真机验证 30 秒超时及取消均能恢复，后端会话保持为空。当前网站麦克风权限仍为 `prompt`，需用户处理内置浏览器地址栏外层授权，尚无真实响铃、接通、字幕或双向音频证据。密钥与可选代理仅在本机私密保存；当前草稿分支尚未合入 `main`，同步状态以 Git 核验为准。
...
2026-09-23 本轮麦克风预检与取消修复通过最终构建、76/76 项单元测试、前端两份脚本语法检查及差异检查；此前代理修复的定向 lint、TypeScript 与离线安装计划证据保留。Codex 内置浏览器已实际完成注册、预检等待、30 秒超时和取消验证；Chrome 控制本轮再次约 21 秒后 fetch 失败，单独诊断。
...
本轮重新核对 TwiML App 回调地址与方法匹配，公网健康探针返回 200 且应用标识正确，无签名 `/voice/client` 返回 403，最近 45 分钟 Twilio Calls 为空。独立本机诊断显示安全上下文与 `mediaDevices` 可用、麦克风权限为 `prompt`，`getUserMedia` 等待 15 秒仍未完成；当前卡在麦克风授权/准备阶段，该诊断不经过 Twilio、不上传音频。
...
前端现先预检麦克风，再建立后端会话。真机点击拨号后已显示“等待麦克风授权，请查看地址栏…”及“取消准备”，此时本机 `activeSession: null`、Twilio 对应查询为空；约 30 秒后显示 `MICROPHONE_TIMEOUT` 并恢复拨号。再次准备时点击取消也立即恢复，麦克风未启用、结束按钮禁用。预检所得流只交给 SDK 使用一次，取消后的延迟结果及旧 Device 连接会被隔离；这些验证不代表授权后电话已经接通。

当前仍等待用户处理网站麦克风授权，之后继续验证设备打开、SDK 连接和真实音频。若缺少本机访问令牌，设置页现在也会显示全局错误及重新从桌面入口打开的指引，已真机核验。测试号码、浏览器身份和临时诊断页面不写入共享文档。
